...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    acceptableFileTypes: [],        // A list of file extensions to accept.
  ...
  return function(req, res) {
    ...
    // NUM-specific changes - check file extensions are valid.
    if (corsAnywhere.acceptableFileTypes && corsAnywhere.acceptableFileTypes.length > 0) {
      var l = location.path.toLowerCase();

      var ok = false;
      corsAnywhere.acceptableFileTypes.forEach(function (ft) {
        if (l.endsWith(ft)) {
          ok = true;
        }
      });
      ...
      if (!ok) {
        res.writeHead(403, 'Forbidden', cors_headers);
        res.end('The url is not allowed.');
        return;
      }
    }
